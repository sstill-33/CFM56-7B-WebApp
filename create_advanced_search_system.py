#!/usr/bin/env python3
"""
Advanced CFM56-7B Search System Generator
Creates a comprehensive search system with part number linking and image relationships
"""
...
def extract_part_numbers_from_xml(xml_file):
    """Extract part numbers and their relationships from XML files"""
    parts = []
    try:
        ...
        for item in root.findall('.//item'):
            ...
            if pnr_elem is not None:
                part_info['part_number'] = pnr_elem.text
                
                # Extract part name/description
                ...
                figure_elem = item.getparent().getparent()  # Go up to figure level
                if figure_elem is not None and figure_elem.tag == 'figure':
                    part_info['figure_ref'] = figure_elem.get('key', '')
                    part_info['figure_title'] = figure_elem.find('title')
                    if part_info['figure_title'] is not None:
                        part_info['figure_title'] = part_info['figure_title'].text
                
                # Extract image file reference
                graphic_elem = item.find('.//graphic')
                if graphic_elem is not None:
                    sheet_elem = graphic_elem.find('.//sheet')
                    if sheet_elem is not None:
                        part_info['image_file'] = sheet_elem.get('uncfile', '')
                        part_info['sheet_number'] = sheet_elem.get('sheetnbr', '')
...
def extract_service_bulletins_from_csv(csv_file):
    """Extract service bulletin information from CSV"""
    service_bulletins = []
    try:
...
def build_search_database():
    ...
    print("Building comprehensive search database...")
    ...
    # Extract parts from EIPC XML files
    print("Extracting part numbers from EIPC files...")
    eipc_xml_files = glob.glob(f"{base_dir}/datas/data/EIPC/7B/*.xml")
    for xml_file in eipc_xml_files:
        parts = extract_part_numbers_from_xml(xml_file)
        for part in parts:
            part['source_file'] = xml_file
            part['category'] = 'EIPC'
    ...
    if os.path.exists(sb_csv):
        search_db['service_bulletins'] = extract_service_bulletins_from_csv(sb_csv)
...
def create_advanced_html_viewer(search_db):
    """Create advanced HTML viewer with search functionality"""
...
<head>
    ...
    <title>CFM56-7B Advanced Search System</title>
    <style>
        body {{
...
<body>
    <div class="container">
        <div class="header">
            <h1>CFM56-7B Advanced Search System</h1>
                ...
                searchDatabase.parts.forEach(part => {{
                    if (part.part_number && part.part_number.toLowerCase().includes(query)) {{
                        results.push({{
                            ...
                            details: `Figure: ${{part.figure_ref || 'N/A'}} | Chapter: ${{part.chapter}}-${{part.section}}-${{part.unit}}`,
                            actions: [
...
"""
    ...
    # Write the HTML file
    with open(f"{output_dir}/advanced_search.html", "w", encoding="utf-8") as f:
        f.write(html_content)
    
    print(f"Advanced search viewer created: {output_dir}/advanced_search.html")
    return f"{output_dir}/advanced_search.html"

def main():
    print("=== CFM56-7B Advanced Search System Generator ===")
    
    # Build search database
    search_db = build_search_database()
    
    # Create advanced HTML viewer
    html_file = create_advanced_html_viewer(search_db)
    
    print(f"\n✅ Advanced search system created!")
    print(f"   Main viewer: /Users/stevenstill/Desktop/Programming/Cursor Apps/EM/CFM56-7B_Extracted/index.html")
    print(f"   Advanced search: {html_file}")
    print(f"\nFeatures:")
    print(f"   - Search by part numbers (e.g., '9324M60G01')")
    print(f"   - Search by keywords (e.g., 'engine', 'fan', 'turbine')")
